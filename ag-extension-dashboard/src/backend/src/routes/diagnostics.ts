...
async function runDnsChecks(domain: string): Promise<Record<string, unknown>> {
    try {
        ...
        return { error: `DNS check failed: ${(err as Error).message}` };
    }
}

async function runPortChecks(): Promise<Array<{ port: number; name: string; host: string; open: boolean }>> {
    const portsToCheck = [
...
async function runTraefikChecks(): Promise<Record<string, unknown>> {
    try {
...
async function runContainerChecks(): Promise<Array<{ name: string; port: number; reachable: boolean }>> {
...
async function runSslChecks(sslDomain: string): Promise<Record<string, unknown>> {
...
function getSummaryAndRecommendations(results: any, domain: string): { issues: string[]; recommendations: string[]; summary: string } {
    ...
    if (results.dns?.[domain] && !results.dns[domain].resolved) {
        issues.push(`DNS resolution failed for ${domain}`);
...
/**
 * GET / — Runs comprehensive infrastructure checks.
 */
router.get('/', async (_req: Request, res: Response) => {
    try {
        ...
        res.json({ success: true, cached: false, timestamp: results.timestamp, ...results });
    } catch (error) {
