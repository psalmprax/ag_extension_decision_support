...
**Cache:** `workbox` `CacheFirst` `ml-models` `30d` in `vite.config.ts:103`, `public/models/` copied to `dist/models/` (not precached to keep install 2818 KiB)
...
3. Replace `public/models/plant-disease.onnx` and bump `modelVersion: 'plant-disease-onnx-v2'` in `edgePlantVisionClassifier.ts:475`
