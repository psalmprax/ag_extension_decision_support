...
import { Users, Send, FileText, Trash2, X, Lock } from 'lucide-react';
...
export const PortfolioPage: React.FC<PortfolioPageProps> = ({
  ...
  return (
    <div>
      ...
      {isDemo && (
        <div
          className={`mb-6 p-6 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-800 ${radiusClass} flex items-start gap-4`}
        >
          ...
          <div>
            <h3 className="text-base font-bold text-amber-900 dark:text-amber-100 mb-1">
              This option is not available in Free-version
            </h3>
            <p className="text-sm text-amber-800 dark:text-amber-200">
              Portfolio management with bulk SMS, export, and full farmer records is available on
              paid plans.
            </p>
          </div>
        </div>
      ...
      {selectedFarmers.size > 0 && (
        <div
          className={`mb-4 p-4 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 ${radiusClass}`}
        >
      ...
      {showBulkSmsComposer && selectedFarmers.size > 0 && (
        <div
          className={`mb-4 p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 ${radiusClass} shadow-sm`}
        >
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 ${radiusClass} bg-gradient-to-br from-primary-500 to-blue-500 shadow-lg shadow-primary-500/20 flex flex-shrink-0 items-center justify-center text-white font-black text-lg`}
                  >
                    {farmer.firstName?.[0]}
                  </div>
                  <div>
                ...
                <div
                  className="pt-1 pr-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 focus-within:opacity-100"
              ...
              <div className="flex-1 space-y-4 my-2">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xxs font-black uppercase tracking-widest text-gray-400 mb-1">
                      {t('table_region_village')}
                    </p>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-800 dark:text-gray-100">
                      {farmer.farmSize}{' '}
                      <span className="text-xs text-gray-500 font-medium tracking-normal">ha</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 min-h-[30px]">
                  {farmer.crops?.map((crop: string) => (
                    <span
                      key={crop}
                      className={`px-2.5 py-1 bg-gray-100/50 dark:bg-gray-800 text-primary-600 dark:text-primary-300 ${radiusClass} text-xxs font-bold uppercase tracking-tight border border-gray-200 dark:border-gray-700 shadow-sm`}
                    >
                      {crop}
                    </span>
                  ))}
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-white/10 flex justify-between items-center">
                <span className="px-3 py-1 bg-green-500/10 text-green-600 dark:text-green-400 rounded-full text-xxs font-black uppercase tracking-widest shadow-inner shadow-green-500/20">
                  {t('table_active')}
                </span>
                <div className="flex -space-x-2">
                  <div className="w-6 h-6 rounded-full bg-blue-100 border-2 border-white dark:border-gray-800 flex items-center justify-center text-xxs font-bold text-blue-600">
                    SMS
                  </div>
                </div>
              </div>
