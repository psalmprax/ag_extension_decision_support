...
export function DiseaseLibraryTab({
    allDiseases, radiusClass, onViewDiseaseInfo
}: Props) {
    ...
    return (
        <div className="card p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Disease Encyclopedia</h3>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Browse or search our comprehensive database of crop diseases.</p>
                </div>
                    ...
                    {allDiseases
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <h4 className="font-semibold text-gray-900 dark:text-white line-clamp-1">{disease}</h4>
                            </div>
                        </motion.div>
                    ))}
                    {allDiseases.filter(disease => disease.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
