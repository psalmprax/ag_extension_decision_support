...
export const fetchEmailTemplates = async (category?: string): Promise<{ success: boolean; data: EmailTemplate[] }> => {
    const url = category ? `/email/templates?category=${category}` : '/email/templates';
    const response = await apiClient.get(url);
    return response.data;
};

export const fetchPendingApprovals = async (): Promise<{ success: boolean; data: EmailApproval[] }> => {
    const response = await apiClient.get('/email/approvals/pending');
    return response.data;
};

export const approveEmail = async (approvalId: string, comment?: string): Promise<{ success: boolean }> => {
