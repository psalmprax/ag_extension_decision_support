...
export const fetchHandoffLog = async (): Promise<{ success: boolean; data: Array<{ from: string; to: string; taskId: string; reason: string; timestamp: string }> }> => {
    const response = await apiClient.get('/ai/agents/handoffs');
    return response.data;
};

export const dispatchTask = async (task: { type: string; payload: Record<string, unknown>; priority?: string; agentId?: string }): Promise<{ success: boolean; data: AgentTask }> => {
