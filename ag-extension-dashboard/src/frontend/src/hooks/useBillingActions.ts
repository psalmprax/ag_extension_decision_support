...
interface SubscribeErrorResponse {
  ...
  currentSubscription?: {
    plan?: { stripePriceId?: string; [k: string]: unknown };
    [k: string]: unknown;
  };
  [key: string]: unknown;
}
...
function handleSubscribeError(
  ...
  if (response.errorCode === 'ALREADY_SUBSCRIBED') {
    ...
    setConfirmModal({
      ...
      message: canSchedule
        ? `${msg} ${t('confirm_next_cycle')}`
        : `${msg} ${t('confirm_reenable_renewal')}`,
      variant: 'warning',
...
export function useBillingActions() {
  ...
  const [mobilePayData, setMobilePayData] = useState({
    method: 'mpesa' as 'mpesa' | 'airtel' | 'bank',
    transactionId: '',
    planId: '',
    amount: '',
  });
  ...
  const handleRedeemVoucher = async () => {
    ...
    try {
      ...
      if (res.success) {
        setFormMessage({
          type: 'success',
          text: res.message || `Successfully activated ${res.data?.planName || 'plan'}!`,
        });
        setVoucherCode('');
      ...
      setFormMessage({
        type: 'error',
        text: err?.response?.data?.message || 'Voucher redemption failed.',
      });
    } finally {
  ...
  const handleSubmitTransaction = async (): Promise<void> => {
    ...
    try {
      ...
      if (res.success) {
        setFormMessage({
          type: 'success',
          text: res.message || 'Transaction submitted for verification!',
        });
  ...
  const handleSwitch = async (priceId: string, billingCycle: 'current' | 'next' = 'current') => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(
          data.errorCode === 'PAYMENT_GATEWAY_NOT_CONFIGURED'
            ? t('billing_configuration_alert') || data.message
            : data.message || 'Failed to switch plan'
        );
      }
    } catch (error: unknown) {
  ...
  const handlePortal = async () => {
    ...
    try {
      ...
      if (data.success && data.data.url) window.location.href = data.data.url;
        ...
        toast.error(
          data.errorCode === 'PAYMENT_GATEWAY_NOT_CONFIGURED'
            ? t('billing_configuration_alert') || data.message || 'Billing portal unavailable.'
  ...
  const handleAddMethod = async () => {
    ...
    try {
      ...
      if (response.success && response.data?.url) window.location.href = response.data.url;
        ...
        toast.error(
          response.errorCode === 'PAYMENT_GATEWAY_NOT_CONFIGURED'
            ? t('billing_configuration_alert') || response.message
            : response.error || response.message || 'Failed to initialize payment method setup'
        );
    } catch (error: unknown) {
    ...
    try {
      ...
      if (response.success && response.data?.approvalUrl)
        ...
        toast.error(
          response.errorCode === 'PAYMENT_GATEWAY_NOT_CONFIGURED'
            ? t('billing_configuration_alert') || response.message
            : response.message || 'Failed to initiate PayPal subscription'
        );
    } catch (error) {
