...
function drawAmbientGlow(ctx: CanvasRenderingContext2D, width: number, height: number, step: number): void {
...
export const LiquidBackgroundCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (e: PointerEvent) => {
      ...
      lastX = e.clientX;
      lastY = e.clientY;
      createFluidSplat(e.clientX, e.clientY, dx, dy, particles);
    };
    ...
    const handleTouchMove = (e: TouchEvent) => {
      if (e.touches.length > 0) {
        ...
        lastX = touch.clientX;
        lastY = touch.clientY;
        createFluidSplat(touch.clientX, touch.clientY, dx, dy, particles);
      }
    };
    ...
    const render = () => {
      ...
      drawAmbientGlow(ctx, width, height, step);
