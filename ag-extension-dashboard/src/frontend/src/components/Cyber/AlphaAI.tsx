...
const SystemStatsGrid = ({
  ...
  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    {systemStats.map((stat, i) => (
      <div
        key={i}
        className={`p-6 bg-white/5 border border-white/5 ${radiusClass} group hover:border-white/10 transition-all cursor-default relative overflow-hidden`}
      >
        <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
          <stat.icon className="w-12 h-12" />
        </div>
        <div className="flex items-center justify-between mb-2">
          <span className="text-micro font-black text-white/40 uppercase tracking-widest">
            {stat.label}
          </span>
          <span
...
const TerminalPanel = ({
  ...
  handleTerminalCommand: (c: string) => void;
  radiusClass: string;
}) => (
  <AnimatePresence>
    {showTerminal && (
...
const useTerminal = (healthData: Record<string, unknown> | undefined) => {
  ...
  const [terminalOutput, setTerminalOutput] = React.useState<string[]>([
    'Alpha AI Terminal v4.2 — Type "help" for commands',
  ]);
    ...
    try {
      ...
      if (data.success && data.data?.agents) {
        ...
        setTerminalOutput(prev => [
          ...prev,
          'Agent status: 3 registered (Agent Zero, Crew AI, OpenClaw)',
        ]);
      }
    } catch {
      setTerminalOutput(prev => [
        ...prev,
        'Agent status endpoint unavailable — agents registered but status unknown',
      ]);
    }
    setIsTerminalLoading(false);
  };
  ...
  const handleTerminalCommand = async (cmd: string) => {
    ...
    switch (trimmed) {
      case 'help':
        setTerminalOutput(prev => [
          ...prev,
          'Commands: help, status, health, agents, uptime, clear',
        ]);
        break;
      case 'status':
      case 'health':
      ...
      case 'uptime':
        if (healthData?.uptime) {
          setTerminalOutput(prev => [
            ...prev,
            `System uptime: ${formatUptime(healthData.uptime as number)}`,
          ]);
        } else {
          setTerminalOutput(prev => [
            ...prev,
            'Uptime data unavailable — health endpoint not responding',
          ]);
        }
        break;
      case 'clear':
...
const TopNavigationBar = ({
  ...
  modes: Array<{ id: string; name: string; icon: React.ElementType; color: string }>;
  activeMode: string;
  setActiveMode: (m: 'actionable' | 'ops') => void;
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 mb-12">
      <div className="flex items-center gap-6">
        ...
        <div className="flex gap-2">
          {modes.map(mode => (
            ...
            >
              <mode.icon
                className={`w-4 h-4 ${activeMode === mode.id ? mode.color : 'text-white/20'}`}
              />
              {mode.name}
            </button>
          ))}
      ...
      <div className="flex items-center gap-4">
        <div
          className={`flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 ${radiusClass}`}
        >
