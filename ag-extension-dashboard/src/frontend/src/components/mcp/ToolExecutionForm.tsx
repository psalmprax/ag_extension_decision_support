      ...
      case 'number':
        return (
          <input
            ...
            value={value}
            onChange={e =>
              onArgsChange({ ...args, [propertyName]: parseFloat(e.target.value) || 0 })
            }
            placeholder={schema.description || `Enter ${propertyName}`}
            className={`w-full px-3 py-2 border border-gray-300 dark:border-gray-600 ${radiusClass} bg-white dark:bg-gray-800 text-gray-900 dark:text-white`}
          />
      ...
      case 'boolean':
        return (
          <select
            ...
            onChange={e =>
              onArgsChange({ ...args, [propertyName]: e.target.value === SCHEMA_BOOLEAN_TRUE })
            }
            className={`w-full px-3 py-2 border border-gray-300 dark:border-gray-600 ${radiusClass} bg-white dark:bg-gray-800 text-gray-900 dark:text-white`}
          >
            ...
            <div className="space-y-2">
              {executionResult.content.map((item, index) => (
                <div key={index} className="text-sm">
                  {item.type === 'text' && (
                    <pre
                      className={`whitespace-pre-wrap font-mono text-gray-900 dark:text-white bg-white dark:bg-gray-800 p-2 border ${radiusClass}`}
                    >
                      {item.text}
                    </pre>
                  )}
