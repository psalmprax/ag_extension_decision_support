...
const ErrorState = ({ error, t, isHost, handleStartCall, handleJoinCall }: { error: string, t: (k: string) => string, isHost: boolean, handleStartCall: () => void, handleJoinCall: () => void }) => (
...
const PreCallView = ({ t, participants, localStream, localVideoRef, isHost, isJoining, handleStartCall, handleJoinCall }: { t: (k: string) => string, participants: Record<string, unknown>[], localStream: MediaStream | null, localVideoRef: React.RefObject<HTMLVideoElement>, isHost: boolean, isJoining: boolean, handleStartCall: () => void, handleJoinCall: () => void }) => (
...
const LocalVideo = ({ localStream, isVideoEnabled, localVideoRef, userName, t, isHost }: { localStream: MediaStream | null, isVideoEnabled: boolean, localVideoRef: React.RefObject<HTMLVideoElement>, userName: string, t: (k: string) => string, isHost: boolean }) => (
...
export function VideoCall({ roomId, userId, userName, isHost = false, onEnd }: VideoCallProps) {
    ...
    if (error) {
        return <ErrorState error={error} t={t} isHost={isHost} handleStartCall={handleStartCall} handleJoinCall={handleJoinCall} />;
    }
    ...
    return (
        <div className="bg-gray-900 rounded-lg overflow-hidden">
            <div className="grid grid-cols-2 gap-2 p-4" style={{ minHeight: '400px' }}>
                <LocalVideo localStream={localStream} isVideoEnabled={isVideoEnabled} localVideoRef={localVideoRef} userName={userName} t={t} isHost={isHost} />
            ...
            <div className="bg-gray-800 px-4 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={toggleAudio} className={`p-3 rounded-full ${isAudioEnabled ? 'bg-gray-700 hover:bg-gray-600 text-white' : 'bg-error-600 hover:bg-error-700 text-white'}`} title={isAudioEnabled ? (t('video_mute') || 'Mute') : (t('video_unmute') || 'Unmute')}>
                        {isAudioEnabled ? <Mic className="w-5 h-5" /> : <MicOff className="w-5 h-5" />}
                    </button>
                    <button onClick={toggleVideo} className={`p-3 rounded-full ${isVideoEnabled ? 'bg-gray-700 hover:bg-gray-600 text-white' : 'bg-error-600 hover:bg-error-700 text-white'}`} title={isVideoEnabled ? (t('video_camera_off') || 'Turn off camera') : (t('video_camera_on') || 'Turn on camera')}>
                        {isVideoEnabled ? <Video className="w-5 h-5" /> : <VideoOff className="w-5 h-5" />}
                    </button>
                    <button onClick={handleEnd} className="p-3 rounded-full bg-error-600 hover:bg-error-700 text-white" title={t('video_end_call') || 'End call'}>
                        <PhoneOff className="w-5 h-5" />
                    </button>
                    {!isHost && (
                        <button onClick={handleLeave} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg">
                            {t('video_leave') || 'Leave'}
                        </button>
                    )}
                </div>
