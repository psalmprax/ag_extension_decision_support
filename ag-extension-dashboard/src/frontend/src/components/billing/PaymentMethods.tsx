...
export const PaymentMethods: React.FC<PaymentMethodsProps> = ({
  ...
  return (
    <section
      ...
      className={`card p-10 bg-white dark:bg-gray-900 border-none shadow-2xl ${radiusClass} group overflow-hidden relative`}
    >
      ...
      <div className="flex justify-between items-center mb-10 relative z-10">
        <div className="flex items-center gap-4">
          <div
            className={`p-3 bg-primary-500/10 ${radiusClass} shadow-inner group-hover:bg-primary-500 transition-colors duration-500`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
        ...
        <div
          className={`p-6 ${radiusClass} bg-green-500/5 border border-green-500/20 group/pm hover:border-green-500/40 transition-all duration-300 relative overflow-hidden`}
        >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 relative z-10">
              <div
                className={`w-12 h-12 bg-green-500 ${radiusClass} flex items-center justify-center shadow-lg shadow-green-500/20`}
              >
              ...
              <div className="space-y-0.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xxs font-black text-gray-400 uppercase tracking-widest">
                    {t('billing_mobile_transfer') || 'Mobile Transfer'}
                  </p>
                </div>
              </div>
            </div>
          ...
          {showMobilePayForm && (
            <div className="mt-4 pt-4 border-t border-green-500/20 space-y-3">
              ...
              <Input
                ...
                onChange={e =>
                  setMobilePayData({ ...mobilePayData, transactionId: e.target.value })
                }
                placeholder="Enter M-Pesa/Airtel Transaction ID"
              />
              <div className="flex items-center justify-between">
                <p className="text-xxs text-gray-400">
                  Admin will verify your payment before activation.
                </p>
                <Button
                  loading={actionLoading === 'mobile-pay'}
                  disabled={!mobilePayData.transactionId || !mobilePayData.planId}
                  onClick={handleSubmitTransaction}
                  className="bg-green-500 text-white hover:bg-green-600 font-black uppercase tracking-widest text-xxs"
                >
                  Submit Transaction
                </Button>
              </div>
              {formMessage && (
                <p
                  className={`text-xs font-medium ${formMessage.type === 'success' ? 'text-green-600' : 'text-red-500'}`}
                >
                  {formMessage.text}
                </p>
              )}
        ...
        {/* AgriVoucher Option */}
        <div
          className={`p-6 ${radiusClass} bg-indigo-500/5 border border-indigo-500/20 group/pm hover:border-indigo-500/40 transition-all duration-300 relative overflow-hidden`}
        >
          <div className="absolute top-0 right-0 p-2 opacity-10">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 relative z-10">
              <div
                className={`w-12 h-12 bg-indigo-500 ${radiusClass} flex items-center justify-center shadow-lg shadow-indigo-500/20`}
              >
                <Ticket className="w-6 h-6 text-white" />
              </div>
              <div className="space-y-0.5">
                <p className="text-sm font-black text-gray-900 dark:text-white tracking-tight">
                  {t('billing_voucher') || 'AgriVoucher'}
                </p>
                <p className="text-xxs font-black text-gray-400 uppercase tracking-widest">
                  {t('billing_voucher_desc') || 'Prepaid Service Code'}
                </p>
              </div>
            </div>
            <Button
            ...
            <div className="mt-4 pt-4 border-t border-indigo-500/20 space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="bg-indigo-500 text-white hover:bg-indigo-600 font-black uppercase tracking-widest text-xxs"
                >
                  Activate Voucher
                </Button>
              </div>
        ...
        {paymentMethods.length > 0 ? (
          paymentMethods.map(pm => (
            <div
              key={pm.id}
              className={`p-6 ${radiusClass} bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800 flex items-center justify-between group/pm hover:border-primary-500/30 transition-all duration-300`}
            >
              <div className="flex items-center gap-4">
                <div
              ...
              <button
                onClick={() => pm.id && onDeleteMethod(pm.id)}
          ...
          <div
            className={`col-span-2 p-10 border-2 border-dashed border-gray-100 dark:border-white/5 ${radiusClass} flex flex-col items-center justify-center gap-4 opacity-40`}
          >
            <Lock className="w-8 h-8" />
            <p className="text-xxs font-black uppercase tracking-[0.2em]">
              {t('billing_no_secure_methods')}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="mt-8 pt-8 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            <h4 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tighter">
              {t('billing_paypal_gateway')}
            </h4>
            <p className="text-xxs font-black text-gray-400 uppercase tracking-widest">
              {t('billing_global_p2p')}
            </p>
          </div>
        </div>
        <Button
          ...
          onClick={() => onPayPalSubscription('price_pro_monthly')}
