...
export const SubscriptionStatus: React.FC<SubscriptionStatusProps> = ({
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <motion.div
            ...
            className="absolute -top-1/4 -right-1/4 w-full h-full bg-emerald-500/15 rounded-full blur-[100px]"
          />
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-2.5 text-slate-400 font-mono text-xs p-3 rounded-xl bg-slate-900/60 border border-slate-800/80">
        ...
        <div className="relative z-10 pt-6 border-t border-slate-800 space-y-3">
          <button
            ...
            className={`w-full h-14 bg-slate-900 hover:bg-slate-800 border border-slate-700/80 text-white ${radiusClass} font-black uppercase tracking-widest text-xs transition-all duration-300 flex items-center justify-center gap-2.5 shadow-xl active:scale-[0.98] disabled:opacity-50 group/btn`}
          >
            {actionLoading === 'portal' ? (
              <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" />
            ) : (
              <ExternalLink className="w-4 h-4 text-emerald-400 group-hover/btn:translate-x-0.5 transition-transform" />
            )}
  ...
  return (
    <motion.section
      initial={{ opacity: 0, scale: 0.96 }}
      animate={{ opacity: 1, scale: 1 }}
      className={`relative p-8 lg:p-10 bg-slate-950/90 dark:bg-slate-950/95 backdrop-blur-2xl ${radiusClass} border border-emerald-500/25 shadow-2xl overflow-hidden min-h-[440px] flex flex-col justify-between`}
    >
      ...
      <div className="relative z-10 p-4 bg-slate-900/80 rounded-xl border border-slate-800">
        <div className="flex items-center gap-2 text-emerald-400 font-black text-xxs uppercase tracking-widest mb-2">
          <Sparkles className="w-3.5 h-3.5" />
          <span>{t('billing_instant_activation') || 'INSTANT COOPERATIVE ACTIVATION'}</span>
        </div>
