                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {invoices.length > 0 ? invoices.map((invoice) => (
                            <tr key={invoice.id} className="hover:bg-primary-50/30 dark:hover:bg-primary-900/10 transition-colors duration-300 group/row">
                                ...
                                <td className="px-10 py-6"><Badge variant={invoice.status === 'paid' ? 'success' : 'warning'} size="sm"><div className={`w-1.5 h-1.5 rounded-full mr-1 ${invoice.status === 'paid' ? 'bg-green-500 shadow-[0_0_8px_var(--color-outline)]' : 'bg-yellow-500'}`} />{invoice.status === 'paid' ? t('billing_status_paid') : invoice.status}</Badge></td>
