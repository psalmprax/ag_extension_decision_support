  ...
  return (
    <section>
      ...
      <div className="space-y-4">
        {safeVisits.length > 0 ? (
          safeVisits.map((visit, i) => (
            <div key={visit.id || i} className="relative pl-8 group">
              {i !== safeVisits.length - 1 && (
                <div
                  className={`absolute left-3 top-6 bottom-[-16px] w-0.5 transition-colors ${isCyber ? 'bg-primary-500/30' : 'bg-gray-100 dark:bg-gray-800 group-hover:bg-primary-500'}`}
                />
              )}
              <div
                className={`absolute left-0 top-1.5 w-6 h-6 rounded-full border-4 shadow-md border-white dark:border-gray-900
                             ${visit.status === 'completed' ? (isCyber ? 'bg-primary-500 neon-glow-primary' : 'bg-primary-500') : 'bg-accent-500'}`}
              />

              <div
                className={`p-4 ${radiusClass} border transition-all cursor-pointer ${
                  isCyber
                    ? 'bg-primary-500/5 hover:border-primary-500/30'
          ...
          <div
            className={`p-8 text-center ${radiusClass} border border-dashed bg-gray-50/50 dark:bg-gray-800/30 border-gray-200 dark:border-gray-700`}
          >
