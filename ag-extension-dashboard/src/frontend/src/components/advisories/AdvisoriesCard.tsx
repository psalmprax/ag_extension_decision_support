...
const RULE_LABEL: Record<string, string> = {
    ...
    dry_spell_warning: 'Dry spell',
    faw_degree_day: 'Fall armyworm risk',
    late_blight_risk: 'Late blight risk',
};
    ...
    return (
        <div className={`${cardClass} p-4 sm:p-5`}>
            <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-black uppercase tracking-widest">Proactive Advisories</h3>
            </div>

            {isLoading ? (
                <div className="h-20 animate-pulse bg-gray-100 dark:bg-gray-800 rounded-xl" data-testid="advisories-skeleton" />
            ) : !data || data.length === 0 ? (
                <p className="text-xs text-gray-400">No advisories dispatched yet. The engine runs daily when farmers opt in.</p>
            ) : (
                            ...
                            <li key={`${d.ruleKey}-${d.district}-${d.dispatchedAt}-${i}`} className="flex gap-2.5">
                                ...
                                <div className="min-w-0">
                                    <p className="text-xs font-bold">
                                        {RULE_LABEL[d.ruleKey] || d.ruleKey}
                                        <span className="text-gray-400 font-normal"> · {d.district} · {d.audienceCount} farmers</span>
                                    </p>
