...
const HistoryTabContent = ({
  ...
  <section className="space-y-6">
    <div className="flex items-center justify-between">
      <h3
        className={`text-sm font-black uppercase tracking-[0.2em] flex items-center gap-2 text-gray-400`}
      >
...
const CropsSection = ({
  ...
  <section
    className={`p-6 ${radiusClass} border shadow-sm bg-theme-bg-card dark:bg-gray-800/80 border-gray-100 dark:border-gray-700`}
  >
    <h3
      className={`text-xs font-black uppercase tracking-widest mb-4 flex items-center gap-2 text-gray-400`}
    >
    ...
    {isEditing ? (
            ...
            <Badge key={i} variant="info" size="sm" className="flex items-center gap-1">
              ...
              <button
                ...
                onClick={() =>
                  setEditData({
                    ...editData,
                    crops: (editData.crops as string[])?.filter(
            ...
            onKeyDown={e => {
              if (e.key === 'Enter') {
                ...
                if (val && !((editData.crops as string[]) || []).includes(val)) {
                  setEditData({
                    ...editData,
                    crops: [...((editData.crops as string[]) || []), val],
                  });
                  input.value = '';
                }
              }
      ...
      <div className="flex flex-wrap gap-2">
        {(farmer.crops || []).map((crop: string, i: number) => (
          <Badge key={`${crop}-${i}`} variant="info" size="sm">
            {crop}
          </Badge>
        ))}
      </div>
  ...
  >
    ...
    <div className={`w-full h-1.5 rounded-full mt-3 overflow-hidden bg-gray-100 dark:bg-gray-700`}>
      <div
        ...
        style={{ width: `${farmer.vitalScore || 0}%` }}
      />
    </div>
    {isEditing && (
      <div className="mt-4">
...
const OverviewTabContent = ({
  ...
  setEditData: (d: Record<string, unknown>) => void;
  visits?: Visit[];
  handleUpdateVisitStatus: (id: string, st: string) => void;
  ...
  <>
      ...
      {[
        ...
        <button
          ...
          onContextMenu={e => {
            e.preventDefault();
            (
              showContextMenu as unknown as (data: {
                x: number;
                y: number;
                entityType: string;
                entityId?: string;
    ...
    <FarmerVisitTimeline
      visits={visits}
      handleUpdateVisitStatus={
        handleUpdateVisitStatus as unknown as (id: string, st: string) => void
      }
      radiusClass={radiusClass}
      isCyber={isCyber}
    />
...
export const FarmerDetailPanel: React.FC<FarmerDetailPanelProps> = ({
  ...
  const [localVisits, setLocalVisits] = React.useState<Visit[]>(
    (visits ?? []) as unknown as Visit[]
  );
