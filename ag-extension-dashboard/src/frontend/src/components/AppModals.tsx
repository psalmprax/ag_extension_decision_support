...
export const AppModals: React.FC<AppModalsProps> = props => {
  ...
  return (
    <>
            ...
            >
              ...
              <div>
                ...
                <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">
                  {t('reports_generating_desc') ||
                    'Our AI is analyzing visit records, yield trends, and farmer interactions to generate your comprehensive report.'}
                </p>
              </div>
            ...
            <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                  {props.viewingReport.title}
                </h3>
                <p className="text-xs text-gray-500 mt-1">
                  Generated: {new Date(props.viewingReport.generatedAt).toLocaleString()} · Status:{' '}
                  {props.viewingReport.status}
                </p>
              </div>
              <button
                onClick={() => {
      ...
      {/* Farmer Selection Modal */}
      {(props.activeTab === 'aiassistant' || props.activeTab === 'farmerchat') &&
        props.showFarmerModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div
              className="absolute inset-0 bg-black/50 backdrop-blur-sm"
              onClick={() => props.setShowFarmerModal(false)}
            />
            <div
              className={`relative bg-white dark:bg-gray-800 ${radiusClass} shadow-2xl w-full max-w-lg max-h-[80vh] overflow-hidden`}
            >
              <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white">{t('chat_start_new')}</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {t('chat_select_farmer')}
                  </p>
                </div>
                <button
                  onClick={() => props.setShowFarmerModal(false)}
              ...
              <div className="p-2 overflow-y-auto max-h-96">
                ...
                ) : props.farmerList.filter(
                    f =>
                      !props.farmerSearchQuery ||
                      `${f.firstName} ${f.lastName}`
                        .toLowerCase()
                        .includes(props.farmerSearchQuery.toLowerCase()) ||
                      (f.region || '')
                        .toLowerCase()
                        .includes(props.farmerSearchQuery.toLowerCase()) ||
                      (f.village || '')
                  ...
                  props.farmerList
                    ...
                    .map(farmer => (
                      <button
                        key={farmer.id}
                        onClick={() =>
                          props.handleStartConversation(
                            farmer,
                            props.activeTab === 'farmerchat' ? 'farmer' : 'ai'
                          )
                        }
