    ...
    return (
        <div className={`markdown-content prose prose-slate dark:prose-invert max-w-none ${className}`}>
            <ReactMarkdown 
                ...
                components={{
                    // Style headers
                    h1: ({ children }) => <h1 className="text-2xl font-black uppercase tracking-tight mb-4 text-gray-900 dark:text-white">{children}</h1>,
                    h2: ({ children }) => <h2 className="text-xl font-bold uppercase tracking-wide mt-6 mb-3 text-primary-600 dark:text-primary-400 border-l-4 border-primary-500 pl-3">{children}</h2>,
                    ...
                    th: ({ children }) => <th className="px-4 py-3 text-left text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">{children}</th>,
                    td: ({ children }) => <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 border-t border-gray-100 dark:border-gray-800">{children}</td>,
                    
                    // Support for checklists
                    input: ({ checked }) => (
                        <span className="inline-block mr-2">
                            {checked ? <CheckCircle className="w-4 h-4 text-green-500 inline" /> : <AlertCircle className="w-4 h-4 text-gray-400 inline" />}
                        </span>
                    ),
                    ...
                    // Code blocks
                    code: (props) => {
                        const { node: _node, inline, children, ...rest } = props as { node?: unknown; inline?: boolean; children?: React.ReactNode; [key: string]: unknown };
                        return inline 
