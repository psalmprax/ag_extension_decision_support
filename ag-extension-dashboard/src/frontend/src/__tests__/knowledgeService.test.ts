...
import { searchKnowledge, askAI, fetchKnowledgeHistory, fetchKnowledgeStats } from '@/api/knowledgeService';
    ...
    describe('searchKnowledge', () => {
        it('should search with query parameter', async () => {
            const mockResponse = {
                data: {
                    ...
                    data: {
                        articles: [
                            { id: '1', title: 'Maize Disease', content: 'Test content', score: 0.9 }
                        ],
                        total: 1,
                    },
                },
        ...
        it('should search with category and crop filters', async () => {
            ...
            expect(mockGet).toHaveBeenCalledWith(
                expect.stringContaining('category=Crop+Management')
            );
            expect(mockGet).toHaveBeenCalledWith(
                expect.stringContaining('crop=maize')
            );
        });
