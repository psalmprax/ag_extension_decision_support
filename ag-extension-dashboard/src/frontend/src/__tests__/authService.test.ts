...
describe('authService', () => {
    ...
    describe('edge cases — input validation', () => {
        it('should handle empty email gracefully', async () => {
            mockPost.mockRejectedValue({
                response: { status: 400, data: { error: 'Email is required' } },
            });
        ...
        it('should handle empty password gracefully', async () => {
            mockPost.mockRejectedValue({
                response: { status: 400, data: { error: 'Password is required' } },
            });
        ...
        it('should handle very long email address (200+ chars)', async () => {
            const longEmail = 'a'.repeat(200) + '@test.com';
            mockPost.mockRejectedValue({
                response: { status: 400, data: { error: 'Email too long' } },
            });

            await expect(
                login({ email: longEmail, password: 'pass123' })
            ).rejects.toBeDefined();
        });
        ...
        it('should handle very long password (1000+ chars)', async () => {
            ...
            mockPost.mockRejectedValue({
                response: { status: 400, data: { error: 'Password too long' } },
            });

            await expect(
                login({ email: 'test@test.com', password: longPassword })
            ).rejects.toBeDefined();
        });
        ...
        it('should handle malformed success response missing token', async () => {
            mockPost.mockResolvedValue({
                data: {
                    success: true,
                    data: {
                        user: { id: '1', email: 't@t.com', role: 'farmer' as const, firstName: 'T', lastName: 'U' },
                    },
                },
            });

            const result = await login({ email: 't@t.com', password: 'pass' });
            expect(result.data.token).toBeUndefined();
            expect(result.data.user.email).toBe('t@t.com');
        });

        it('should handle concurrent login attempts independently', async () => {
            mockPost
                .mockResolvedValueOnce(mockAuthResponse)
                .mockRejectedValueOnce(new Error('Concurrent request failed'));
