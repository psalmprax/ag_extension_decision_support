...
test.describe('Theme Comparison: Cyber vs Forest', () => {
    ...
    test('should verify interactive elements work in both themes', async ({ page }) => {
        ...
        const themeButtonVisibleForest = await themeButton.isVisible({ timeout: 3000 }).catch(() => false);
        ...
        const themeButtonVisibleCyber = await themeButton.isVisible({ timeout: 3000 }).catch(() => false);
