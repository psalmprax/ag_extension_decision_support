...
- **Threat**: External data sources (such as syndicated agronomic feeds, untrusted farmer WhatsApp memos, or external web tool results) containing hidden instructions (`<|im_start|>`, `ignore previous instructions`, `send env keys`).
- **Controls Implemented**:
  ...
  - **Perimeter Gate Neutralization**: In [`securityGate.ts`](file:///home/psalmprax/ALL_PROJECTS/ag_extension_decision_support/ag-extension-dashboard/src/backend/src/middleware/securityGate.ts), legitimate media payloads (`audio`, `imageData`, `video`) are safely neutralized to `[SANITIZED_MEDIA_PAYLOAD]` for threat scanning, preventing false positives while catching malicious text injections in headers, queries, and parameters.
